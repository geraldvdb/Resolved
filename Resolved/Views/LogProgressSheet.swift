...
struct LogProgressSheet: View {
    ...
    var onRewardsUnlocked: (([Reward]) -> Void)?
                    ...
                    ScrollView {
                        VStack(spacing: 24) {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                TextField("", text: $descriptionText, prompt: Text("How did it go?").foregroundColor(AppColors.secondaryText(colorScheme).opacity(0.6)))
    ...
    private func logProgress() {
        ...
        // Notify parent about unlocked rewards
        if !newlyUnlocked.isEmpty {
            onRewardsUnlocked?(newlyUnlocked)
        }
